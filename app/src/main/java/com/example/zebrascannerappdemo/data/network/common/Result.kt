...
sealed class Result<out T> {
    ...
    sealed class Success<T> : Result<T>() {
        ...
        ) : Success<T>(), com.example.zebrascannerappdemo.data.network.common.HttpResponse
    ...
    sealed class Failure<E : Throwable>(open val error: E? = null) : Result<Nothing>() {
        ...
        class NetworkError(override val error: Throwable) : Failure<Throwable>(error)
        ...
        class BadRequestError(
            override val error: BadRequestException
        ) : Failure<BadRequestException>(), HttpResponse {
